...
(defcustom flycheck-relevant-error-other-file-min-level 'error
...
whose error level is at least as severe as this one list.  If
nil, display all errors from other files."
  :group 'flycheck
  :type 'flycheck-minimum-level
  :safe #'flycheck-error-level-p
  :package-version '(flycheck . "32"))
  ...
  (let ((file-name (flycheck-error-filename err)))
    (and file-name
         ...
         (<= (flycheck-error-level-severity
              flycheck-relevant-error-other-file-min-level)
